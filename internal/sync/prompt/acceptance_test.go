...
func (api *directAPI) MakeRequest(
	...
	if strings.Contains(path, "/sync/plan") || strings.Contains(path, "/sync/apply") {
		return nil, fmt.Errorf("sync endpoint must not be called")
	}
	...
	case "GET":
		...
		if api.variation != nil {
			variations = append(variations, *api.variation)
		}
...
func TestPromptFirstSyncCreatesUpsertVariation(t *testing.T) {
	...
	assertManifestFingerprint(t, root, local)
	assertNoSyncEndpoints(t, api.requests)
}
...
func assertNoSyncEndpoints(t *testing.T, requests []string) {
	t.Helper()
	for _, request := range requests {
		assert.NotContains(t, request, "/ai-configs/sync/plan")
		assert.NotContains(t, request, "/ai-configs/sync/apply")
	}
}
