...
func (state model) View() string {
	if state.err != nil || state.canceled {
		return ""
	}
	...
	case selectProject:
		...
		return state.projects.View()

	case selectConfig:
		if !state.configsReady {
			return loadingView("Loading configs")
		}
		return state.configs.View()

	case selectVariations:
		if !state.variationsReady {
			return loadingView("Loading prompt variations")
		}

		var view strings.Builder
		view.WriteString(state.variations.View())
		selected, _ := state.variationCounts()
		_, _ = fmt.Fprintf(&view, "\nSelected: %d", selected)
		if state.notice != "" {
			view.WriteString("\n")
			view.WriteString(noticeStyle.Render(state.notice))
		}
