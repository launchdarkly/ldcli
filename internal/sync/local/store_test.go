	...
	resources := []VariationFile{
		{
			...
			Variation: syncdomain.Variation{
				...
				Messages: []syncdomain.Message{
					{Role: "system", Content: `Explain <system>nested</system> tags from C:\prompts.`},
...
func TestStore_AddRollsBackNewFilesWhenOneExists(t *testing.T) {
	...
	newVariation := localVariation("new")
	newVariation.ProjectKey = "new-project"
	_, err = store.Add([]VariationFile{newVariation, existing})
	require.ErrorIs(t, err, ErrVariationExists)

	exists, err := store.VariationExists("new-project", "config", "new")
	require.NoError(t, err)
	assert.False(t, exists)
	_, statErr := os.Stat(filepath.Join(root, syncdomain.RootDir, "new-project"))
	require.ErrorIs(t, statErr, os.ErrNotExist)
}

func TestStore_AddValidationFailureLeavesNoWorkspace(t *testing.T) {
	root := t.TempDir()
	resource := localVariation("../unsafe")

	_, err := NewStore(root).Add([]VariationFile{resource})
...
func TestStore_ReplaceVariationsPreservesLocalMetadata(t *testing.T) {
	root := t.TempDir()
	store := NewStore(root)
	existing := localVariation("existing")
	existing.Upsert = false
	_, err := store.Add([]VariationFile{existing})
	require.NoError(t, err)
	...
	server := existing.Variation
	server.Name = "Changed on server"
	server.ModelConfigKey = "claude"
	server.ModelConfigVersion = 3
	server.Messages = []syncdomain.Message{{
		Role:    "system",
		Content: "Use the server instructions.",
	}}
	...
	require.NoError(t, err)
	assert.Equal(t, []string{"project/configs/config/existing.prompt.md"}, paths)
...
func TestStore_ReplaceVariationsPreflightsBeforeWriting(t *testing.T) {
	root := t.TempDir()
	store := NewStore(root)
	existing := localVariation("existing")
	_, err := store.Add([]VariationFile{existing})
	...
	changed := existing.Variation
	changed.Name = "Changed on server"
...
func TestStore_DeleteVariationsRemovesFileAndEmptyWorkspace(t *testing.T) {
	root := t.TempDir()
	store := NewStore(root)
	_, err := store.Add([]VariationFile{localVariation("existing")})
	require.NoError(t, err)

	paths, err := store.DeleteVariations([]VariationDeletion{{
		ProjectKey:   "project",
		ConfigKey:    "config",
		VariationKey: "existing",
	}})
...
func TestStore_DeleteVariationsPreflightsBeforeDeleting(t *testing.T) {
	root := t.TempDir()
	store := NewStore(root)
	_, err := store.Add([]VariationFile{localVariation("existing")})
	require.NoError(t, err)

	_, err = store.DeleteVariations([]VariationDeletion{
		{ProjectKey: "project", ConfigKey: "config", VariationKey: "existing"},
		{ProjectKey: "project", ConfigKey: "config", VariationKey: "missing"},
	})

	require.ErrorIs(t, err, os.ErrNotExist)
	exists, existsErr := store.VariationExists("project", "config", "existing")
