...
func TestDetachResourcesRollsBackNewManifestWhenWrapperDeletionFails(t *testing.T) {
	root := t.TempDir()
	...
	require.ErrorContains(t, err, "parse variation")
	_, exists, loadErr := manifestStore.Load()
...
func TestResourceModelSelectsMultipleResources(t *testing.T) {
	resources := []Resource{
		{Kind: syncdomain.KindVariation, ProjectKey: "project", LookupKey: "config/one"},
		{Kind: syncdomain.KindVariation, ProjectKey: "project", LookupKey: "config/two"},
	}
	model := newModel(resources)

	model.toggleSelected()
	assert.Equal(t, []Resource{resources[0]}, model.selectedResources())
	model.selectAll()
	assert.Equal(t, resources, model.selectedResources())
}
