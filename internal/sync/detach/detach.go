...
func Run(options Options) error {
	...
	if len(resources) == 0 {
		_, _ = fmt.Fprintln(options.Output, "No resources are currently synced.")
		return nil
	}
	...
	program := tea.NewProgram(newModel(resources), tea.WithAltScreen(), tea.WithInput(options.Input), tea.WithOutput(options.Output))
	...
	if !ok {
		return fmt.Errorf("detach returned an unexpected model")
	}
	if result.canceled {
		return nil
	}

	selected := result.selectedResources()
	if err := detachResources(options, manifest, manifestExists, selected); err != nil {
		return err
	}

	_, _ = fmt.Fprintln(options.Output, "Detached resources:")
	for _, resource := range selected {
		_, _ = fmt.Fprintf(options.Output, "- %s %s/%s\n", resource.Kind, resource.ProjectKey, resource.LookupKey)
	}
	return nil
}
