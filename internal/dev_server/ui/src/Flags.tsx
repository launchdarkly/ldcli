...
function Flags({selectedProject}:FlagProps) {
    ...
    const removeOverride = (flagKey: string, updateState: boolean = true) => {
        return fetch(apiRoute(`/dev/projects/${selectedProject}/overrides/${flagKey}`), {
            method: 'DELETE',
        })
            .then((res) => {
                // todo: clean this up.
    ...
    const fetchFlags = async() => {
        const res = await fetch(apiRoute(`/dev/projects/${selectedProject}?expand=overrides`))
    ...
    useEffect(() => {
        fetchFlags().catch(console.error.bind(console, 'error when fetching flags'));
    }, [selectedProject]);
