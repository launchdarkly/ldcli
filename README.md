...
Two steps cannot be automated because they are browser flows: registering and installing the GitHub App, and creating a Kiro API key. In a terminal, `setup` pauses on each one, showing a single URL to open. The Kiro pause takes the key you paste and stores it as the `KIRO_API_KEY` GitHub Actions secret on `--github-owner`/`--github-repo` through the GitHub CLI, and pressing Enter without one skips it; pass `--kiro-api-key` to store a key without pausing. GitHub polls AWS and continues on its own once the registration appears — and connects the repository immediately if you passed `--github-owner`, `--github-repo` and `--github-repo-id`. GitHub is skipped entirely when the account already has a registration, and any key stops it waiting. Pass `--no-wait` to list the steps instead, for example in CI:
