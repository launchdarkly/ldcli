...
Without `--access-token`, `setup` pauses on the LaunchDarkly page where you create a service token and connects the MCP server with the token you paste there, so you do not need to have one ready beforehand. Press Enter at that prompt to skip it, or pass `--skip-mcp-server` to leave the MCP server out entirely.
