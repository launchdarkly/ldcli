...
func NewPromptCmd(client resources.Client) *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: "Bootstrap local prompt variations from LaunchDarkly, add more variations, or synchronize changes using the committed local manifest. " +
			"Sync rechecks state before every write, but the existing variation APIs do not support atomic conditional writes; rerun sync after a concurrent change.",
	...
	cmd.Flags().Bool(watchFlag, false, "Sync when managed prompt files change")
